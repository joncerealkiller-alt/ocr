"""
Tests google/vit-base-patch16-224 (timm tag: vit_base_patch16_224.orig_in21k -
Google's ORIGINAL checkpoint, ImageNet-21k pretrained only, NOT timm's own
"augreg" retrain and NOT fine-tuned down to 1k) as a candidate NEW tower for
the evidence-fusion classifier work, per Jon's direction. NOT one of the 8
QUALIFIED_ENCODERS already validated in the Multi-Tower Routing Audit
(docs/BENCHMARK2_3_MULTI_TOWER_ROUTING_AUDIT.md) - ViT-Base/16 pretrained on
ImageNet-21k's 21,843 object categories (much broader than MobileNetV2's
1000), the question is whether that broader pretraining produces embeddings
that cluster more usefully for THIS project's document taxonomy.
...
similarity methodology as the real Multi-Tower Routing Audit and the
MobileNetV2 test, not a new/different evaluation method invented for
this one model.

Ground truth: `data/outputs/reference_pipeline_v2/manifest_final.csv`
(the real, pre-automation whole-corpus human-reviewed set - see
docs/MULTI_SOURCE_VOTING_CLASSIFIER_PROPOSAL.md's "Resource" section).
Reference set: up to 15 images per category (or all available if fewer).
Test set: up to 30 DIFFERENT images per category (disjoint from the
reference set), held out for accuracy measurement - same reference/
held-out split and same RANDOM_SEED as the MobileNetV2 test, for direct
comparability.

Usage:
    python diagnostics/test_vit_base21k_classification.py
"""
...
TIMM_TAG = "vit_base_patch16_224.orig_in21k"  # matches google/vit-base-patch16-224, 21k classes
N_REFERENCE_PER_BUCKET = 15
N_TEST_PER_BUCKET = 30
RANDOM_SEED = 42
REPORT_PATH = PROJECT_ROOT / "data" / "logs" / "reviewed" / "vit_base21k_classification_report.txt"
...
def main():
    print(f"Loading {TIMM_TAG} (matches google/vit-base-patch16-224)...")
