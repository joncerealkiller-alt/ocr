"""
Ablation requested by Jon (2026-08-07) after the held-out fine-tune result
(89.8% test acc, see docs/MULTI_SOURCE_VOTING_CLASSIFIER_PROPOSAL.md's
"Fine-tuned ViT-21k..." section): that run couldn't separate "fine-tuning
helps" from "484 extra dense_tabular_rows images helps" as the dominant
driver, since both changed at once. This run holds the TECHNIQUE fixed
(identical fine-tune setup, hyperparameters, 3-way held-out split
methodology) and removes ONLY the 484 LAC census pull images, training
dense_tabular_rows on manifest_final.csv's original 168 examples alone -
same as every other category. If held-out accuracy (especially on
dense_tabular_rows specifically) stays close to the 89.8%/92.8% numbers
from the full run, fine-tuning itself is the dominant driver. If it drops
substantially, the extra data was doing most of the work.

Uses a SEPARATE dataset CSV and SEPARATE checkpoint directory/filenames
from the main run, per Jon's explicit instruction not to overwrite the
current best checkpoint - data/outputs/vit21k_doc_classifier_checkpoints/
best.pt and best_holdout_eval.pt are left untouched.

Usage:
    python diagnostics/train_vit21k_ablation_no_census_pull.py
"""
...
ABLATION_DATASET_CSV = PROJECT_ROOT / "data" / "outputs" / "vit_finetune_dataset_ablation_no_census_pull.csv"
TIMM_TAG = "vit_base_patch16_224.orig_in21k"

# separate from the main run's checkpoint dir/files - do NOT touch best.pt / best_holdout_eval.pt
CHECKPOINT_DIR = PROJECT_ROOT / "data" / "outputs" / "vit21k_doc_classifier_checkpoints"
CHECKPOINT_PATH = CHECKPOINT_DIR / "ablation_no_census_pull.pt"
REPORT_PATH = PROJECT_ROOT / "data" / "logs" / "reviewed" / "vit21k_ablation_no_census_pull_report.txt"
...
def build_ablation_dataset() -> dict[str, list[str]]:
    """manifest_final.csv only - no LAC census pull supplement."""
    ...
    with open(ABLATION_DATASET_CSV, "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
...
def main():
    ...
    log(f"Device: {DEVICE}")
    log("ABLATION: dense_tabular_rows uses ONLY manifest_final.csv's original "
        "examples - the 484 LAC census pull images are excluded.")
    log(f"Writing ablation dataset manifest: {ABLATION_DATASET_CSV}")
    by_cat = build_ablation_dataset()
    ...
    log(f"\n=== HELD-OUT TEST ACCURACY (ablation, no census pull supplement): "
    ...
    # separate filename from the main run's checkpoints - never overwrite best.pt / best_holdout_eval.pt
    torch.save({
        ...
        "test_acc": test_acc,
        "note": "ablation: dense_tabular_rows WITHOUT the 484 LAC census pull images",
    }, CHECKPOINT_PATH)
    log(f"\nCheckpoint (ablation, kept separate from main-run checkpoints): {CHECKPOINT_PATH}")
