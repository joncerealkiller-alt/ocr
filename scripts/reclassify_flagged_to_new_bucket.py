...
(data/buckets/<old_bucket>.csv) and INTO the target bucket's CSV
(data/buckets/<target_bucket>.csv, created if it doesn't exist yet),
...
which is overwritten to the target bucket. data/manifest.csv is
untouched (Stage 0 output, has no category column - these files are
still validly in the corpus, just reclassified within Stage 1's output).
...
def _plan(flagged_csv: Path, target_bucket: str) -> tuple[list[dict], list[tuple[str, str]], dict[str, list[str]]]:
    """
...
def reclassify(flagged_csv: Path, target_bucket: str, dry_run: bool = False, assume_yes: bool = False) -> None:
    to_reclassify, skipped, by_old_bucket = _plan(flagged_csv, target_bucket)
    ...
    if not assume_yes:
        print(f"\nWARNING: this will PERMANENTLY remove {len(to_reclassify)} row(s) from "
              f"{len(by_old_bucket)} bucket CSV(s) listed above and add them to "
              f"data/buckets/{target_bucket}.csv (git-tracked, `git diff`/`git checkout` "
              f"recovers them if needed). No image files are moved or deleted.")
...
def main():
    ...
    reclassify(csv_path, args.target_bucket, dry_run=args.dry_run, assume_yes=args.yes)
