"""
ARCHIVED 2026-07-30 - DO NOT USE, DO NOT IMPORT. Moved from scripts/ to
scripts/archive/ (git history preserved via `git mv`) after a full
dependency audit confirmed zero live references anywhere in the repo.

Jon: "build_manifest was the initial starting module. its legacy now
and wont be used going forward." Its role is now split across two real
Stage 0 entry points, both built on core/manifest_pipeline.py's
build_working_manifest_from_paths() (copies + deskews + preprocesses,
not just a raw path list):
  - scripts/build_working_manifest.py - folder-picker CLI (this file's
    direct successor).
  - scripts/run_preprocessing.py - CSV-input adapter, for an arbitrary/
    multi-source file selection (e.g. ui/build_manifest_ui.py's queued
    files) rather than one folder.

Original docstring, kept for archival record:
