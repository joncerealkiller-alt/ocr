...
Usage:
    python scripts/auto_generate_sidecar.py <dewarped_image.jpg> \\
        [--out PATH] [--debug] [--force]

Default output: data/outputs/auto_row_segmentation/<stem>_sidecar.json -
a SEPARATE directory from data/outputs/row_segmentation/ (where manual
sidecars live), specifically so this experimental tool can never
collide with or silently overwrite real manually-segmented work. --out
overrides the path directly; --force allows overwriting an existing
file at the resolved path (refused by default either way).
...
def main():
    ...
    parser.add_argument("--out", type=str, default=None,
                         help="Output sidecar JSON path. Default: "
                              "data/outputs/auto_row_segmentation/<stem>_sidecar.json")
    ...
    if args.out:
        ...
        out_path = repo_root / "data" / "outputs" / "auto_row_segmentation" / f"{stem}_sidecar.json"
